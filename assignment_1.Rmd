...
Now, to calculate $E[Y]$, we evaluate $G'_Y(\theta)|_{\theta=1}  = [\frac{2}{3^2} + 4\theta^3\frac{2}{3^3} + 9\theta^8\frac{2}{3^4}]|_{\theta=1}$\
$=\frac{2}{9}+\frac{4\cdot2}{27}+\frac{9\cdot2}{81} = 0.7407$ **Ans**
...
For $G_X(\theta)$ to be a valid PGF, $0\le(1-2\lambda)\le1 \implies 0 < \lambda\le\frac{1}{2} \implies \lambda \in [0,\frac{1}{2})$ **Ans**
...
$1-2\lambda= e\cdot(1-\lambda e-\lambda e^2)$
$1-2\lambda = e - \lambda e^2 - \lambda e^3$
$\lambda e^3 + \lambda e^2 - e - 2\lambda + 1=0$
...
```{r}
v <- c(0,1,2)
sum(4^v/factorial(v)*exp(-4)*(1 - 2^v*exp(-4)))
```
...
$\implies G_Z(\theta) = \frac{e^{(\lambda+\mu)\theta} - e^{\lambda\theta} - e^{\mu\theta} + 1}{(e^{\lambda} - 1)(e^{\lambda} - 1)}$

$\implies G_Z(\theta) = \frac{1}{(e^{\lambda} - 1)(e^{\lambda} - 1)}[1 + \frac{(\lambda+\mu)\theta}{1!} + \frac{(\lambda+\mu)^2\theta^2}{2!} \frac{(\lambda+\mu)^3\theta^3}{3!} +  .... - (1 + \frac{(\lambda)\theta}{1!} + \frac{(\lambda)^2\theta^2}{2!} \frac{(\lambda)^3\theta^3}{3!} + ....) - (1 + \frac{(\mu)\theta}{1!} + \frac{(\mu)^2\theta^2}{2!} \frac{(\mu)^3\theta^3}{3!} + ....) + 1]$

$\implies G_Z(\theta) = \frac{1}{(e^{\lambda} - 1)(e^{\lambda} - 1)}[\theta\frac{(\lambda + \mu) - \lambda - \mu}{1!} + \theta^2\frac{(\lambda + \mu)^2 - \lambda^2 - \mu^2}{2!} + \theta^3\frac{(\lambda + \mu)^3 - \lambda^3 - \mu^3}{3!} + ......]$


$\implies G_Z(\theta) = \frac{1}{(e^{\lambda} - 1)(e^{\lambda} - 1)}[\theta^2\frac{(\lambda + \mu)^2 - \lambda^2 - \mu^2}{2!} + \theta^3\frac{(\lambda + \mu)^3 - \lambda^3 - \mu^3}{3!} + ......]$

Following the above pattern, we can say that $P(Z=k)$ is the coefficient of $\theta^k$. Therefore,

$P(Z=k) = \frac{1}{k!(e^{\lambda} - 1)(e^{\lambda} - 1)}[(\lambda + \mu)^k - \lambda^k - \mu^k]$

This can be written as required in the question. 

$P(Z=k) = C(\lambda,\mu, k)[(\lambda + \mu)^k - \lambda^k - \mu^k]$, where $C(\lambda,\mu, k) = \frac{1}{k!(e^{\lambda} - 1)(e^{\lambda} - 1)}$ **Ans**
